== ValidateThis Rules Configuration File ==

Each business object will have its own corresponding rules configuration file, in which all of the validation rules for the object are described.

The xml schema file can be found at: [http://www.validatethis.org/validateThis.xsd /ValidateThis/core/validateThis.xsd].

It is '''highly recommended''' that when creating a new rules xml file, you validate your xml against this xml schema.
							...
							<li>
								...
								<ul>
									<li>
										...
										''{any}'' Each param must have a single attribute, which is a value expected by its corresponding rule. 
										E.g., a MaxLength rule would require a param with an attribute maxlength="x", where x is the maxlength to test for.
									</li>
								</ul>
							</li>
						</ul>
